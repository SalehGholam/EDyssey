...
def default_analysis_save_dir(fn_nav):
    ...
    `fn_nav` sits inside a "<project>_EDyssey Analysis/navigator signal/
    <timestamp>/" folder - the structure the navigator tab's own save
    produces - reuse that same "<project>_EDyssey Analysis" folder, so this
    ...
    generic "EDyssey Analysis" folder next to the file, as before."""
    d = os.path.dirname(fn_nav)            # .../<timestamp>
    parent = os.path.dirname(d)            # .../navigator signal
    grandparent = os.path.dirname(parent)  # .../<project>_EDyssey Analysis
